...
- **异步处理模式**: 支持通过 Mistral Batch API 进行后台异步处理，为大批量任务提供 50% 的费用节省。
- **隐私优先设计**: 
  - **API 密钥本地化**: 用户的 Mistral AI API 密钥存储在浏览器本地（localStorage），永不上传到中间服务器。
  - **直接通信**: 密钥仅随请求发送至 Cloudflare Worker，并直接提交给 Mistral AI。
  - **取件码机制**: 引入个人取件码，支持跨设备、跨会话恢复处理任务。
- **极简 UI/UX**: 基于 React + Tailwind CSS v4 构建的高端、现代设计，现已全面完成中文化及布局优化。
- **高质量 OCR**: 采用 Mistral AI 提供的专业级 OCR 技术，自动提取表格、分级标题等复杂结构。

## 📍 版本记录

### v2.0.0 (当前版本)
- **异步处理模式**: 引入“异步模式”（原批量模式），采用 Mistral Batch API 实现后台异步识别。
- **50% 成本节省**: 利用批量处理定价，大幅降低 API 调用费用。
- **会话持久化**: 实现“取件码”功能，允许用户在不同设备或浏览器会话中找回异步任务。
- **全面本地化**: UI 全面中文化，优化了数据隐私声明的布局与展示。
- **重大修复**: 解决了 Mistral API 422 错误，并修复了异步任务列表的显示与过滤逻辑。
