...
### v2.1.0 (当前版本)
- **Mistral 直传模式**: 实现了前端直接上传文件至 Mistral API，完全绕过 Cloudflare Worker 的内存限制，彻底解决了大文件（>50MB）导致的 OOM 错误。
- **架构升级**: Worker 现在直接接收 `mistral_file_id` 进行处理，显著提升了系统的稳定性和传输速度。
- **错误处理增强**: 优化了上传和处理阶段的错误捕获逻辑，提升了异常情况下的用户体验。
- **页脚更新**: 更新版权年份至 2026 年，并同步了版本号显示。
