...
- **法律文档脱敏工具**: 集成强大的交互式脱敏工具，自动识别并遮蔽姓名、身份证、电话、金额等敏感信息。支持分类管理并同时导出脱敏文档及对照报告。
...
- **PWA 支持**: 支持安装到桌面或手机，提供类似原生应用的流畅体验。
- **异步处理能力**: 后台异步 OCR 处理，支持任务持久化。引入**取件码系统**，可在不同设备或会话中找回任务进度。
- **大文件自动分割**: 前端自动识别超过 50MB 的 PDF 文件并进行物理分割，确保在常规模式下即便开启智能润色也能稳定处理不超时。
- **隐私与性能并重**: API Key 仅存储于本地，支持大文件直传 Mistral API，绕过 Worker 限制，确保数据流转安全透明。

## 📍 版本记录

### v5.0.0 (当前版本)
- **法律文档脱敏工具**: 推出全新的脱敏功能套件。支持自动识别姓名、证件号、银行账号等 15+ 类敏感信息，并提供专业的交互式处理界面。
- **脱敏对照报告**: 自动生成独立的 Markdown 报告，记录所有脱敏项及其原文对照，为法律合规提供完整审计追踪。
- **专业脱敏界面**: 全新全屏工具，包含分类过滤、批量确认以及脱敏效果实时预览功能。
